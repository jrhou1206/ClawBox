...
import { getClawBoxProviderStore } from '../services/providers/store-instance';
import {
...
export async function storeApiKey(providerId: string, apiKey: string): Promise<boolean> {
  try {
    ...
    const s = await getClawBoxProviderStore();
 ...
 * Also synchronizes ClawBox local provider list with OpenClaw's actual config.
 */
...
> {
  ...
  for (const provider of providers) {
    ...
    // we should remove it from ClawBox UI to stay consistent.
    ...
    if (configMissing || (!isBuiltin && !isActive)) {
      console.log(`[Sync] Provider ${provider.id} (${provider.type}) missing from OpenClaw, dropping from ClawBox UI`);
