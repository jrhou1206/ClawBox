...
<p align="center">
  <a href="#功能特性">功能特性</a> •
  <a href="#为什么选择-clawbox">为什么选择 ClawBox</a> •
  <a href="#快速上手">快速上手</a> •
  <a href="#系统架构">系统架构</a> •
  <a href="#开发指南">开发指南</a> •
  <a href="#参与贡献">参与贡献</a>
...
**ClawBox** 是连接强大 AI 智能体与普通用户之间的桥梁。基于 [OpenClaw](https://github.com/OpenClaw) 构建，它将命令行式的 AI 编排转变为易用、美观的桌面体验——无需使用终端。

无论是自动化工作流、连接通讯软件，还是调度智能定时任务，ClawBox 都能提供高效易用的图形界面，帮助你充分发挥 AI 智能体的能力。

ClawBox 预置了最佳实践的模型供应商配置，原生支持 Windows 平台以及多语言设置。当然，你也可以通过 **设置 → 高级 → 开发者模式** 来进行精细的高级配置。

> 项目说明：当前仓库是基于 [ValueCell-ai/ClawX](https://github.com/ValueCell-ai/ClawX) 完成的二次开发版本，在上游桌面智能体基础上，针对本项目的外壳、工作流、本地化和集成行为做了定制化改造。
...
构建 AI 智能体不应该需要精通命令行。ClawBox 的设计理念很简单：**强大的技术值得拥有一个尊重用户时间的界面。**
...
ClawBox 直接基于官方 **OpenClaw** 核心构建。无需单独安装，我们将运行时嵌入应用内部，提供开箱即用的无缝体验。
...
当你使用 `@agent` 选择其他智能体时，ClawBox 会直接切换到该智能体自己的对话上下文，而不是经过默认智能体转发。各 Agent 工作区默认彼此分离，但更强的运行时隔离仍取决于 OpenClaw 的 sandbox 配置。
...
ClawBox 还会内置预装完整的文档处理技能（`pdf`、`xlsx`、`docx`、`pptx`），在启动时自动部署到托管技能目录（默认 `~/.openclaw/skills`），并在首次安装时默认启用。额外预装技能（`find-skills`、`self-improving-agent`、`tavily-search`、`brave-web-search`）也会默认启用；若缺少必需的 API Key，OpenClaw 会在运行时给出配置错误提示。  
...
### 🌙 自适应主题
支持浅色模式、深色模式或跟随系统主题。ClawBox 自动适应你的偏好设置。

### 🚀 开机启动控制
在 **设置 → 通用** 中，你可以开启 **开机自动启动**，让 ClawBox 在系统登录后自动启动。
...
当前仓库没有单独维护公开发布渠道。如果你拿到的是内部打包版本，请通过对应的内部发布渠道安装；否则请直接使用当前检出的仓库源码进行构建。

#### 从源码构建

```bash
# 在当前仓库目录中`r`n`r`n# 初始化项目
pnpm run init
...
首次启动 ClawBox 时，**设置向导** 将引导你完成以下步骤：
...
> Moonshot（Kimi）说明：ClawBox 默认保持开启 Kimi 的 web search。  
> 当配置 Moonshot 后，ClawBox 也会将 OpenClaw 配置中的 Kimi web search 同步到中国区端点（`https://api.moonshot.cn/v1`）。

### 代理设置

ClawBox 内置了代理设置，适用于需要通过本地代理客户端访问外网的场景，包括 Electron 本身、OpenClaw Gateway，以及 Telegram 这类频道的联网请求。
...
- 如果启用了 Telegram，ClawBox 还会把代理同步到 OpenClaw 的 Telegram 频道配置中。
...
ClawBox 采用 **双进程 + Host API 统一接入架构**。渲染进程只调用统一客户端抽象，协议选择与进程生命周期由 Electron 主进程统一管理：
...
我们欢迎社区的各种贡献！无论是修复 Bug、开发新功能、改进文档还是翻译——每一份贡献都让 ClawBox 变得更好。

### 如何贡献

1. 在你的本地克隆上**创建**功能分支（`git checkout -b feature/amazing-feature`）
2. **提交** 清晰描述的变更
3. **推送** 到你的分支或内部远程仓库
4. 按照你的协作流程创建 **Pull Request / Merge Request**
...
ClawBox 构建于以下优秀的开源项目之上：

- [OpenClaw](https://github.com/OpenClaw) – AI 智能体运行时
- [ValueCell-ai/ClawX](https://github.com/ValueCell-ai/ClawX) – 当前仓库所基于的上游桌面项目
...
本项目基于 [ValueCell-ai/ClawX](https://github.com/ValueCell-ai/ClawX) 二次开发完成。

当前仓库聚焦于项目定制化改造与集成需求；如需查看上游的公开发布、版本说明与社区信息，请参考原始上游项目。

---

## 许可证

ClawBox 基于 [MIT 许可证](LICENSE) 发布。你可以自由地使用、修改和分发本软件。
