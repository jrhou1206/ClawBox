...
# Post-installation script for ClawBox on Linux
...
# Create symbolic link for ClawBox app binary
if [ -x /opt/ClawBox/clawbox ]; then
    ln -sf /opt/ClawBox/clawbox /usr/local/bin/clawbox 2>/dev/null || true
fi

# Create symbolic link for openclaw CLI
OPENCLAW_WRAPPER="/opt/ClawBox/resources/cli/openclaw"
...
if ! { [[ -L /proc/self/ns/user ]] && unshare --user true; }; then
    ...
    chmod 4755 '/opt/ClawBox/chrome-sandbox' || true
else
    chmod 0755 '/opt/ClawBox/chrome-sandbox' || true
fi
...
if apparmor_status --enabled > /dev/null 2>&1; then
    APPARMOR_PROFILE_SOURCE='/opt/ClawBox/resources/apparmor-profile'
    APPARMOR_PROFILE_TARGET='/etc/apparmor.d/clawbox'
...
echo "ClawBox has been installed successfully."
